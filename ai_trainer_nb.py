...
class Personal_Trainer:
	...
	def __init__(self, keys, auto_start=False, opt=False):
		self.keys = keys
		...
		if auto_start:
			if 'squat' in keys:
				if self.opt:
					...
					self.classifiers['squat'] = pickle.load(open(os.path.join('../inference/','squat_classifiers.p'),'rb'))
			if 'pushup' in keys:
				if self.opt:
					self.classifiers['pushup'] = pickle.load(open(os.path.join('../inference/','pushup_classifiers_ftopt.p'),'rb'))
				else: 
					self.classifiers['pushup'] = pickle.load(open(os.path.join('../inference/','pushup_classifiers.p'),'rb'))
	...
	def analyze_reps(self, exercise, data_file, labels=None, epsilon=0.15, gamma=20, delta=0.5, beta=1, auto_analyze=False, verbose=False):
		...
		if self.opt:
			...
			feature_vectors = self.get_prediction_features(exercise, reps)
